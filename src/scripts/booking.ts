...
export async function renderBooking(root: HTMLElement): Promise<void> {
	...
	root.addEventListener("submit", async (e) => {
		...
		try {
			...
			const intake = form.querySelector<HTMLFormElement>("[data-intake] form");
			if (intake) {
				const data: Record<string, unknown> = {};
				const fd = new FormData(intake);
				for (const [k, v] of fd) if (typeof v === "string") data[k] = data[k] === undefined ? v : ([] as string[]).concat(data[k] as string[], v);
...
async function mountIntake(host: HTMLElement, formId: string): Promise<void> {
	try {
		const r = await fetch(`${FORMS}/definition?formId=${encodeURIComponent(formId)}`);
		...
		// The widget submits the intake itself; neutralise the form's own submit button and wiring.
		const f = host.querySelector<HTMLFormElement>("form");
		if (f) {
			f.dataset.formId = formId;
			f.removeAttribute("data-ec-form");
			f.querySelector(".ec-form-nav")?.remove();
