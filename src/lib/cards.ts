...
export function productCardHtml(product: Entry): string {
	...
	return `<article class="ec-product-card" data-product="${esc(product.data.id)}"><a href="/products/${esc(product.id)}" class="ec-product-card__media">${img || `<div class="ec-product-card__placeholder" aria-hidden="true"></div>`}</a><div class="ec-product-card__body"><h3 class="ec-product-card__title"><a href="/products/${esc(product.id)}">${esc(product.data.title ?? "")}</a></h3><p class="ec-product-card__price"><span>${esc(price)}</span>${compare}</p><p class="ec-product-card__stock" data-availability="${esc(product.data.id)}" hidden></p><button type="button" class="ec-add-to-cart" data-add-to-cart="${esc(product.id)}" data-product-id="${esc(product.data.id)}" data-title="${esc(product.data.title ?? "")}" data-price="${esc(String(f.price ?? ""))}" data-slug="${esc(product.id)}">Add to cart</button></div></article>`;
}
